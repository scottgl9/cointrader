...
class TradeRange(object):
    def __init__(self, ticker_id=None, tick_size=0.01):
        self.peaks = []
        self.valleys = []
        self.ema_prices= []
        self.ema = EMA(8)
    ...
    def get_indices_by_price(self, prices):
        round_size = '{:f}'.format(self.tick_size).index('1') - 1
        print(round_size)
        ...
        while current_price <= self.major_resistance:
            ...
            current_price = round(current_price + self.tick_size, round_size)
