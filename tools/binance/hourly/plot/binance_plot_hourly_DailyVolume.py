...
def simulate(hkdb, symbol, start_ts, end_ts):
    ...
    for msg in msgs: #get_rows_as_msgs(c):
        ...
        high = float(msg['high'])
        open = float(msg['open'])
        #base_volume = float(msg['base_volume'])
        volume = float(msg['quote_volume'])
        volumes.append(volume)

        dvolume.update(volume)
        dvolume_x_values.append(i)
        dvolume_values.append(dvolume.result)
    ...
    plt.subplot(212)
    fig21, = plt.plot(dvolume_x_values, dvolume_values, label='VOLUME')
