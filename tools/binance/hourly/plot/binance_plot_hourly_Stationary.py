...
def simulate(kdb, symbol, start_ts, end_ts):
    ...
    for msg in msgs:
        ...
        if not prev_close:
            prev_close = math.log1p(close)
        else:
            log_close = math.log1p(close)
            log_diff = log_close - prev_close
            stationary_values.append(log_diff)
            prev_close = log_close
    ...
    plt.subplot(212)
    fig21, = plt.plot(stationary_values, label='Stationary')
