...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', action='store', dest='filename',
                        default='binance_daily_klines.db',
                        help='filename of hourly kline sqlite db')

    parser.add_argument('--start-date', action='store', dest='start_date',
                        default=date_two_years_ago,
                        help='Start date from which to capture daily klines (month/day/year)')

    parser.add_argument('--end-date', action='store', dest='end_date',
                        default=date_today,
                        help='End date from which to capture daily klines (month/day/year)')
    ...
    for symbol in symbol_table_list:
        ...
        if (current_ts - kline_list[-1][0]) > 3600*24*4*1000:
